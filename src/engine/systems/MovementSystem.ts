...
//
// Stamina regen is owned by GameState.tickOverlays (single source of
// truth, gated on value change). We only DRAIN here.
...
export class MovementSystem {
  ...
  fixedUpdate(dt: number): void {
    ...
    if (this.state.stamina < RUN_STAMINA_THRESHOLD) {
      this.input.release("shift");
    }
    const isRunning = this.input.isDown("shift");
    const moving = this.input.isDown("w") || this.input.isDown("a") ||
    ...
    if (isRunning && moving) {
      this.state.setStamina(this.state.stamina - RUN_STAMINA_COST * dt);
    }
  }
}
